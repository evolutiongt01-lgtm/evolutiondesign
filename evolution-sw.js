/* Evolution Design · Navbar + Router App PC/Móvil · v15 */
(() => {
  'use strict';

  /* =========================================================
     MODO EMBEBIDO
     Si esta página vive dentro del router, su navbar no se pinta.
     El documento padre conserva UNA sola barra fija.
     ========================================================= */
  if (window.self !== window.top) {
    ...
    const isMainRoute = value => {
      try {
        const u = new URL(value, location.href);
        let p = u.pathname.replace(/\/+$/,'').replace(/\.html$/,'').toLowerCase();
    ...
    document.addEventListener('click', e => {
      ...
      try {
        ...
        if (u.origin !== location.origin || !isMainRoute(u)) return;

        e.preventDefault();
        parent.postMessage({
          type:'evolution:navigate',
          href:u.href
        },location.origin);
      } catch (_) {}
    },true);
  ...
  body.evo-router-active{overflow:hidden!important}

  body.evo-router-active > :not(evolution-nav):not(.evo-route-frame):not(.evo-route-loader):not(script):not(style){
  ...
  .evo-route-frame.evo-route-ready{
    opacity:1;
    transform:none;
  }

  .evo-route-frame.evo-route-old{
    z-index:2147481900;
  }
  ...
  .evo-route-loader.show{opacity:1}

  .evo-route-loader::before{
    content:"";
    position:absolute;
    inset:0;
    width:34%;
    background:#d4b895;
    box-shadow:0 0 14px rgba(212,184,149,.4);
    animation:evoRouteProgress .72s ease-in-out infinite;
  }

  @keyframes evoRouteProgress{
...
`;
  ...
  class EvolutionNav extends HTMLElement{
    connectedCallback(){
            ...
            <div class="evo-brand">
              <div class="evo-menu-wrap">
                ...
                <ul id="desktopDropdownMenu"
                  ...
                  <li id="linkProyectosMenuDesktop"
                      class="${projectsVisible?'':'d-none'}">

                    <a class="evo-menu-item"
                       href="proyectos.html">
                      Mis Proyectos
                    </a>
      ...
      <nav class="evo-nav evo-mobile mobile-app-nav"
           id="mobileNav">
  ...
  document.addEventListener(
    ...
    e=>{
      ...
      const desktop=
        el.closest('#desktopMenuBtn');

      const mobile=
        el.closest('#mobileMenuBtn');

      const login=
        el.closest('[data-evo-login]');
  ...
  const routePath = value => {
    ...
    try {
      ...
      if(last === 'arquitectura'){
        return '/arquitectura';
      }

      if(last === 'diseno-grafico'){
  ...
  const syncActiveNav = value => {
    ...
    document
      .querySelectorAll(
        'evolution-nav a[data-evolution-nav]'
      )
      .forEach(
        a=>{
